...
from datasets import CanCAM_MEG, HCP_fMRI, Matrix_HCP_fMRI, Noise, TychoConditionalDataset, TychoUnconditionalDataset, NER_BCI, CRCNS_LFP, AJILE_LFP, load_ajile_data, load_ecog_data
from diffusion_model import Diffusion, Trainer
from likelihood_computation import likelihood_experiment, long_likelihood_experiment
from networks import BaseConv, LongConv
from utils.classifier_utils import (
    TensorLabelDataset,
    train_and_test_classifier,
)
from utils.kernels_and_diffusion_utils import (
    OUProcess,
    WhiteNoiseProcess,
    generate_samples,
    single_channelwise_imputations,
)
from utils.utils import config_saver, count_parameters
...
def init_dataset(cfg):
    ...
    if cfg.dataset.self == "cancam":
        data_set = CanCAM_MEG(
            signal_length=cfg.dataset.signal_length,
            rois=cfg.dataset.rois,
            sub_ids=cfg.dataset.sub_ids,
            pca=cfg.dataset.pca,
            pca_sub_ids=cfg.dataset.pca_sub_ids,
            n_components=cfg.dataset.n_components,
            transposed=cfg.dataset.transposed,
            start_index=cfg.dataset.start_index,
            end_index=cfg.dataset.end_index,
            with_class_cond=cfg.dataset.with_class_cond,
            folder_path=cfg.dataset.folder_path,
            pca_folder_path=cfg.dataset.pca_folder_path,
        )
    elif cfg.dataset.self == "hcp_fmri":
        data_set = HCP_fMRI(
            sub_ids=cfg.dataset.sub_ids,
    ...
    elif cfg.dataset.self == "matrix_hcp_fmri":
        data_set = Matrix_HCP_fMRI(
            sub_ids=cfg.dataset.sub_ids,
            with_class_cond=cfg.dataset.with_class_cond,
            folder_path=cfg.dataset.folder_path,
        )
    elif cfg.dataset.self == "noise":
        data_set = Noise(
            time_windows=cfg.dataset.time_windows,
            rois=cfg.dataset.rois,
            signal_length=cfg.dataset.signal_length,
            with_class_cond=cfg.dataset.with_class_cond
        )
    elif cfg.dataset.self == "tycho_unconditional":
    ...
    if not created_train_test_split:
        ...
        print((train_data_set.indices))
        print((test_data_set.indices))
...
def training_and_eval_pipeline(cfg):
    ...
    print("Start diffusion training script")

    assert cfg.base.experiment is not None
    
    if "_" not in str(cfg.base.experiment):
        cfg.base.experiment = str(cfg.base.experiment)[:6] + "_" + str(cfg.base.experiment)[6:]
    
    if cfg.base.tag is None:
        cfg.base.tag = cfg.base.experiment

    print(OC.to_yaml(cfg))
    print("Config correct? Abort if not.")
    #time.sleep(10)
    print("Start run.")
    ...
    # Save numpy array for test_data
#    np.save(
#        file=os.path.join(cfg.base.save_path, cfg.base.experiment, "test_indices.npy"),
#        arr=test_data_set.indices
#    )
#    np.save(
#        file=os.path.join(cfg.base.save_path, cfg.base.experiment, "train_indices.npy"),
#        arr=train_data_set.indices
#    )
    ...
    print(f"{len(train_data_set)}, {len(test_data_set)}")
    ...
    for i in range(cfg.optimizer.num_epochs):
        ...
        if (i + 1) % cfg.dataset.save_epoch == 0 or i == 0:
            print("Saving model")
            config_saver(
                file=diffusion.state_dict(),
                filename=f"model_{i+1}.pkl",
                cfg=cfg,
            )
    ...
    print("Saving cfg and model if specified.")
    ...
    # Save numpy array for test_data
    np.save(
        file=os.path.join(cfg.base.save_path, cfg.base.experiment, "test_data.npy"),
        arr=test_data_set
    )

    print("Generate samples and run experiment!")
